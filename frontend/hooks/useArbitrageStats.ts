...
// Mock 数据生成
const generateMockRevenueFlows = (dailyData: DailyRevenuePoint[]): RevenueFlow[] => {
  const now = Date.now();
  const flows: RevenueFlow[] = [];
  
  const strategies = [
    { type: 'arbitrage' as const, protocol: 'Uniswap', strategy: '交易所内套利' },
  ...
  for (let i = 0; i < 30; i++) {
    ...
    for (let j = 0; j < txCount; j++) {
      ...
      // 将当日收益分配到各笔交易
      const profit = j === txCount - 1 
        ? remainingProfit 
        : Math.random() * remainingProfit * 0.5;
      
      remainingProfit -= profit;
      
      const txTimestamp = now - daysAgo * 24 * 60 * 60 * 1000 - Math.random() * 24 * 60 * 60 * 1000;
...
const getCachedDailyRevenueData = (): DailyRevenuePoint[] => {
  if (!cachedDailyRevenueData) {
    ...
    console.log('✅ Generated dailyRevenueData:', cachedDailyRevenueData.length, 'items');
  }
...
export const useArbitrageStats = () => {
  ...
  const [stats, setStats] = useState<ArbitrageStats>(() => {
    ...
    const totalProfit = dailyRevenueData[dailyRevenueData.length - 1]?.cumulative || 0;
