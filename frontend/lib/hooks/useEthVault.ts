...
import { Address, Abi, Hex, Chain, parseEther, parseUnits, formatUnits } from "viem";
import { usePublicClient, useWalletClient } from "@/hooks/usePublicClient";
import ArbitrageVaultABI from "@/lib/abi/ArbitrageVault.json";
...
export function useEthVault(
  ...
  const [principalShares, setPrincipalShares] = useState<number>(0);
  ...
  const sharesDecimals = opts?.sharesDecimals ?? 18;
  ...
  const refreshStats = useCallback(async () => {
    ...
    try {
      if (vaultAddress) {
        console.log("join valueAddress logic")
        ...
        setPrincipalShares(parseFloat(formatUnits(shares as bigint, 18)));
      }
      const bal = await publicClient.getBalance({ address: owner });
      setWalletEth(parseFloat(formatUnits(bal, 18)));
    } catch {}
  }, [publicClient, owner, vaultAddress, sharesDecimals]);
  ...
  return { deposit, withdraw, loading, error, principalShares, walletEth, refreshStats };
}
