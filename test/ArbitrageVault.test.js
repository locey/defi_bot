...
describe("ArbitrageVault", function () {
  ...
  describe("套利管理接口", function () {
    it("只有 ArbitrageCore 可以调用 approveForArbitrage", async function () {
      ...
      await expect(
        vault.connect(user1).approveForArbitrage(amount)
      ).to.be.revertedWith("Only ArbitrageCore");

      await vault.connect(arbitrageCore).approveForArbitrage(amount);
      const allowance = await weth.allowance(await vault.getAddress(), arbitrageCore.address);
