...
async function main() {
  ...
  // ----------------------------
  // 7. 部署 FlashLoanArbitrage (普通合约)
  // ----------------------------
  const FlashLoanArbitrage = await hre.ethers.getContractFactory("FlashLoanArbitrage");
  const flashLoanArbitrage = await FlashLoanArbitrage.deploy(
    flashLoanRouter.target,     // _flashLoanRouter
    spotArbitrage.target,       // _spotArbitrage
    hre.ethers.ZeroAddress,     // _arbitrageCore - 临时使用零地址，后续更新
  ...
  const arbitrageCore = await hre.upgrades.deployProxy(ArbitrageCore, [
    spotArbitrage.target,       // _spotArbitrage
    flashLoanArbitrage.target,  // _flashLoanArbitrage - 现在使用FlashLoanArbitrage
    PLATFORM_WALLET,            // _platFormWallet
    configManage.target,        // _configManager
  ...
  // ----------------------------
  // 9. 解决循环依赖：更新 FlashLoanArbitrage 的 ArbitrageCore 地址
  // ----------------------------
  await flashLoanArbitrage.setArbitrageCore(arbitrageCore.target);
  console.log("✅ FlashLoanArbitrage ArbitrageCore set to:", arbitrageCore.target);

  // ----------------------------
  // 10. 解决循环依赖：设置 SpotArbitrage 的 ArbitrageCore 地址
  // ----------------------------
  ...
  const deployments = {
    ...
    flashLoanArbitrage: flashLoanArbitrage.target, // 新增
